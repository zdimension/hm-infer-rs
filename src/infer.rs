...
use crate::{BaseType, Environment, SExpr, SExpr::*, BaseType::*};

pub fn analyze<'a>(expr: &SExpr, env: &Environment<'a>, syms: &HashMap<&str, &'a BaseType<'a>>) -> &'a BaseType<'a>
{
                    ...
                    Symbol(n) if n == "letrec" =>
                        {
                            ...
                            {
                                typevar.unify(aux(val, env, &newenv, &ftypes.iter().map(|(_, t)| *t).collect()));
                            }
                    ...
                    Symbol(n) if n == "lambda" =>
                        {
                            ...
                            if rest.is_empty()
                            {
                                ...
                                let mut newenv = syms.clone();
                                newenv.insert(match &head[0] {
                                    Symbol(name) => name.as_str(),
                                    _ => panic!("lambda: expected identifier")
                                }, ptype);
                                let mut newngen = ngen.clone();
                                newngen.push(ptype);
                                env.arena.alloc(TypeOperator("->".to_string(), vec![ptype, aux(body, env, &newenv, &newngen)]))
                            } else {
