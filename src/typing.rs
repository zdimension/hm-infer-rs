        ...
        match ty {
            TypeVariable(_) => {
                if ngen.iter().any(|t| t.contains(ty)) {
                    ...
                    *map.entry(ByAddress(ty))
                        .or_insert_with(|| arena.alloc(TypeVariable(Cell::new(None))))
                }
            }
            TypeOperator(name, args) => arena.alloc(TypeOperator(
                name.clone(),
                args.iter()
                    .map(|arg| arg.duplicate(arena, map, ngen))
                    .collect(),
            )),
        }
    }
...
impl<'a> Display for BaseType<'a> {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        fn aux<'a, 'b>(
            t: &'a BaseType<'b>,
            letter: &Cell<u8>,
            vars: &mut HashMap<ByAddress<&'a BaseType<'b>>, char>,
            f: &mut Formatter,
        ) -> std::fmt::Result {
            let t = t.resolve();
            match t {
                ...
                TypeOperator(name, args) => match name.as_str() {
                    "->" => {
                        write!(f, "(")?;
